    ...
    with torch.no_grad():
        with model.ema_scope():
            ...
            for n in trange(opt.n_iter, desc="Sampling"):
                ...
                x_img= model.encode_first_stage(src_img)
                x_enc,_ =sampler.encode(x_img,cond, 199)
                # ts = torch.full((src_c.shape[0],), 199, device=src_c.device, dtype=torch.long)
                # x_enc=sampler.stochastic_encode(x_img,ts)
                x_enc =x_enc.detach()
                ...
                src_c=target_c
                cond = {"context":src_c, "y":src_label}
                x_samples_ddim = sampler.decode(x_enc, cond, 199)
